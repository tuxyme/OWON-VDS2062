...
public class LibusbJava {
	...
	static {
		...
		final String folder;
		try {
			File jar = new File(LibusbJava.class.getProtectionDomain().getCodeSource().getLocation().toURI());
			folder = jar.getParentFile().getPath();
		...
		final String fname;
		switch (os.charAt(0)) {
			case 'W': fname = "LibusbJava.dll"  ; break;
			case 'L': fname = "libusbJava.so"   ; break;
			case 'M': fname = "libusbJava.dylib"; break;
			default:
				throw new RuntimeException("Unhandled os.name=" + os);
		}

		if (!load(folder, fname) && !load(folder, os, arch, fname))
			System.loadLibrary(os.charAt(0) == 'W' ? "LibusbJava" : "usbJava");
	...
	private static boolean load(String folder, String filename)
	{
	...
	private static boolean load(String folder, String os, String arch, String filename)
	{
		switch (os.charAt(0)) {
			case 'W' :
				os = "win";
		...
		return load(folder + File.separator + os + File.separator + arch, filename);
	}
